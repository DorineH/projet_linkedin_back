...
async def create_saved_job(
    ...
    job_lite = JobOutLite.model_validate(job) if job else None
    return SavedJobOut(
        id=saved.id,
        job_id=saved.job_id,
...
# endpoint to get list of saved job ids for the user 
@router.get("/saved-jobs/ids", response_model=list[int])
...
async def patch_saved_job(
    ...
    saved = await update_saved_job(session, user_id, saved_id, payload) # type: ignore
    if not saved:
